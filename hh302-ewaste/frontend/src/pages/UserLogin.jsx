...
export default function UserLogin() {
	...
	async function submit(e) {
		...
		try {
			...
			localStorage.setItem('auth_user_token', d.token);
			const redirectTo = (location.state && location.state.from) || '/u';
	...
	return (
		<div className="card" style={{ maxWidth: 420, margin: '64px auto' }}>
			...
			<form onSubmit={submit} className="grid" style={{ gridTemplateColumns: '1fr', gap: 8 }}>
				<input className="input" placeholder="User ID" value={username} onChange={e => setUsername(e.target.value)} disabled={loading} />
